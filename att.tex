...
\title{Space-domain stationary filter for P- and S-wave decomposition in isotropic elastic media}
...
P and S wavefields separation/decomposition is essential for generating PP and PS images in elastic reverse time migrations (ERTMs). Traditionally used divergence and curl operators introduce amplitude and phase change of the input elastic wavefield. We propose to use a space domain stationary filter for PS decomposition. The decomposed P- and S-waves have the same phase and amplitude with the original elastic wavefield. 
